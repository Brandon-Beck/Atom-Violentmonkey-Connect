...
import AttemptConcatView from './attempt-concat-view';
...
export default {
  ...
  rebuildTimer: null,
  rebuildList: {},
  attemptConcatView: null,
  modalPanel: null,
  subscriptions: null,
  is_enabled: false,
  activate(state) {
    this.attemptConcatView = new AttemptConcatView(state.attemptConcatViewState);
    this.modalPanel = atom.workspace.addModalPanel({
      item: this.attemptConcatView.getElement(),
      visible: false
    });
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'attempt-concat:toggle': () => this.toggle()
    }));
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      'attempt-concat:generateConfig': () => this.genConfFile()
    }));
  ...
  deactivate() {
    ...
    this.attemptConcatView.destroy();
  },

  serialize() {
    return {
      attemptConcatViewState: this.attemptConcatView.serialize()
    };
  },

  toggle() {
    console.log('AttemptConcat was toggled!');
    if (this.is_enabled) {
